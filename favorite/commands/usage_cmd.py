from rich.console import Console
  ...
  class UsageCommand(ICommand):
    ...
    def execute(self, args: str, ctx: CommandContext) -> None:
      ...
      # Determine active provider via RouterModule
      try:
      ...
      def row(label: str, value: str) -> None:
          ...
          console.print(f"  [dim {DIM}]{label}[/dim {DIM}]{pad}[white]{value}[/white]")

      console.print()
      console.print(f"  [bold {ORANGE}]USAGE[/bold {ORANGE}]  [dim {DIM2}]{'─' * 36}[/dim {DIM2}]")
      console.print()
